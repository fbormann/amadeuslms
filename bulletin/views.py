...
class NewWindowView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if has_subject_permissions(request.user, bulletin.topic.subject):
            ...
            if difficulties is not None and not difficulties == "":
...
class InsideView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        n_submeteu = False
        for meta in metas:
            if MyGoals.objects.filter(item=meta, user=estudante).exists():
                metas_pessoais.append(MyGoals.objects.get(item=meta, user=estudante))
                n_submeteu = False
            else:
                n_submeteu = True
...
class CreateView(LoginRequiredMixin, LogMixin, generic.edit.CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        pendencies_form = PendenciesForm(initial={'subject': topic.subject.id, 'actions': [("", "-------"),
                                                                                           ("view", _("Visualize"))]})
...
class DeleteView(LoginRequiredMixin, LogMixin, generic.DeleteView):
    ...
    def get_success_url(self):
        ...
        super(DeleteView, self).createLog(self.request.user, self.log_component, self.log_action, self.log_resource,
                                          self.log_context)
...
class StatisticsView(LoginRequiredMixin, LogMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vis_ou = Log.objects.filter(context__contains={'bulletin_id': bulletin.id}, resource="bulletin",
        ...
        if users[0] is not '':
            for u in users:
                ...
                notification = {
                    ...
                    "last_date": _("Last message in %s") % (formats.date_format(created.create_date,
